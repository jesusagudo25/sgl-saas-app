...
import { ArrowLeft, ArrowRight, ArrowUpRight, Building2, CalendarDays, Check, ChevronDown, Eye, FileText, FolderOpen, LayoutDashboard, LogOut, Mail, Menu, Pencil, Plus, Scale, Search, Settings, ShieldCheck, Users, X } from 'lucide-react';
...
import { SettingsCatalogs } from './SettingsCatalogs';
...
function Layout() {
  const { user, logout } = useAuth(); const { activeOrganization: org } = useOrganizations(); const [error, setError] = useState(''); const [menuOpen,setMenuOpen]=useState(false);
  return <div className="app-layout"><button className="drawer-toggle" aria-label="Abrir menú" onClick={()=>setMenuOpen(true)}><Menu/></button>{menuOpen&&<button className="drawer-backdrop" aria-label="Cerrar menú" onClick={()=>setMenuOpen(false)}/>}<aside className={`sidebar ${menuOpen?'open':''}`}><button className="drawer-close" aria-label="Cerrar menú" onClick={()=>setMenuOpen(false)}><X/></button><Brand/><div className="sidebar-section">ESPACIO DE TRABAJO</div>
    ...
    <nav><NavLink to="/app" end><LayoutDashboard size={19}/>Dashboard</NavLink><NavLink to="/app/clients"><Users size={19}/>Clientes</NavLink><NavLink to="/app/cases"><FolderOpen size={19}/>Casos</NavLink>
      ...
      <NavLink to="/app/team"><Users size={19}/>Equipo</NavLink>{org!.roleCode === 'ADMIN' && <NavLink to="/app/settings"><Settings size={19}/>Configuración</NavLink>}</nav>
...
function Clients() {
  ...
  return <><div className="page-heading"><div><span className="eyebrow">GESTIÓN DE CLIENTES</span><h1>Clientes</h1><p className="muted">Personas y empresas de tu organización.</p></div>
    ...
    <section className="panel clients-table responsive-data-table"><div className="table-heading"><h2>Directorio</h2><span className="count">{page?.totalCount ?? 0}</span></div>{loading ? <Loading/> : !page?.items.length ? <div className="empty-state"><Users size={32}/><h3>No hay clientes para mostrar</h3><p>Agrega un cliente o modifica los filtros.</p></div> : <div className="table-scroll"><table><thead><tr><th aria-label="Detalles"/><th>Nombre / Razón social</th><th>Tipo</th><th>Identificación</th><th>Correo</th><th>Teléfono</th><th>Estado</th><th>Última actualización</th><th>Acción</th></tr></thead><tbody>{page.items.map(client => <ResponsiveDataRow key={client.id} id={client.id} cells={[<strong>{client.displayName}</strong>, client.type === 'PERSON' ? 'Persona' : 'Empresa', <>{client.identificationType} · {client.identificationNumber}</>, client.email || '—', client.phone || '—', <span className={`client-status ${client.status.toLowerCase()}`}>{client.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}</span>, new Date(client.updatedAt).toLocaleDateString('es'), <Link className="icon-button" title="Ver cliente" to={`/app/clients/${client.id}`}><Eye size={17}/></Link>]} details={[{label:'Correo',value:client.email},{label:'Teléfono',value:client.phone},{label:'Teléfono secundario',value:client.secondaryPhone},{label:'Dirección',value:client.address},{label:'Última actualización',value:new Date(client.updatedAt).toLocaleDateString('es')}]}/>)}</tbody></table></div>}
...
function ClientForm() {
  ...
  return <><div className="page-heading"><div><Link className="back-link" to={editing ? `/app/clients/${id}` : '/app/clients'}><ArrowLeft size={15}/>Volver</Link><h1>{editing ? 'Editar cliente' : 'Nuevo cliente'}</h1><p className="muted">Información de la persona o empresa.</p></div></div><Alert>{error}</Alert>
    <form className="panel client-form" onSubmit={save}><div className="form-section"><h2>Tipo e identificación</h2><div className="form-row"><label className="field">Tipo<select name="type" value={type} onChange={e => setType(e.target.value as typeof type)}><option value="PERSON">Persona natural</option><option value="COMPANY">Persona jurídica</option></select></label><label className="field">Tipo de identificación<select required name="identificationType" key={`${type}-${client?.identificationType}`} defaultValue={type === 'COMPANY' ? 'RUC' : client?.identificationType ?? 'CEDULA'}>{type === 'PERSON' ? <><option value="CEDULA">Cédula</option><option value="PASSPORT">Pasaporte</option></> : <option value="RUC">RUC</option>}</select></label></div><Field label="Número de identificación" name="identificationNumber" defaultValue={client?.identificationNumber}/></div>
